...
folders = Table(
    ...
    Column("folder_scope_id", Integer, ForeignKey("object_scopes.id", ondelete='SET NULL'),
           index=True, nullable=True),
    Column("full_path", Text, index=True, nullable=False),
    Index('folders_uix_1', 'name',
          'created_by_user', 'parent_directory_id', unique=True),
    Index('folders_uix_2', 'created_by_user', 'full_path', 'folder_state_id', unique=True))
...
files_starred = Table(
    ...
    Column("starred_by",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("is_starred", Boolean, nullable=False),
