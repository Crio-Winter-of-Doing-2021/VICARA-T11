...
DATABASE_URL = "postgresql://postgres:intel@localhost:5432/storage_dev_new"
...
user_states = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column("user_state_type", Text, nullable=False, unique=True))
...
users_info = Table(
    ...
    Column("user_state_id", Integer, ForeignKey("user_states.id", ondelete='SET NULL'),
...
object_states = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column("state_type", Text, nullable=False, unique=True, index=True))

object_scopes = Table(
    "object_scopes", metadata,
    Column("id", Integer, primary_key=True),
    Column("scope_type", Text, nullable=False, unique=True, index=True))

folders = Table(
    "folders", metadata,
    Column("id", Integer, primary_key=True),
    Column("name", Text, nullable=False),
    Column("created_by_user",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("created_on", DateTime, server_default=func.now(),
           nullable=False),
    Column("parent_directory_id", Integer, ForeignKey("folders.id", ondelete='CASCADE'),
           index=True, nullable=True),
    Column("folder_state_id", Integer, ForeignKey("object_states.id", ondelete='SET NULL'),
           index=True, nullable=True),
    Column("folder_scope_id", Integer, ForeignKey("object_scopes.id", ondelete='SET NULL'),
           index=True, nullable=True),
    Index('folders_uix_1', 'name',
          'created_by_user', 'parent_directory_id', unique=True))
...
folders_inactive = Table(
    ...
    Column("folder_id", Integer, ForeignKey("folders.id", ondelete='CASCADE'),
           index=True, nullable=False, unique=True),
    Column("inactive_by_user",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("inactive_on", DateTime, server_default=func.now(),
           nullable=False),
    Column("deleted_from_parent_dir", Integer, ForeignKey("folders.id", ondelete='SET NULL'),
           index=True, nullable=False),
)
...
folders_deleted = Table(
    ...
    Column("deleted_on", DateTime, server_default=func.now(),
           nullable=False),
    Column("deleted_by_user",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
...
content_types = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column("content_type", Text, unique=True, nullable=False))

files = Table(
    "files", metadata,
    Column("id", Integer, primary_key=True),
    Column("name", Text, nullable=False),
    Column("content_type",  Integer, ForeignKey("content_types.id", ondelete='SET NULL'),
           index=True, nullable=False),
    Column("uploaded_by",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("uploaded_on", DateTime, server_default=func.now(), index=True,
           nullable=False),
    Column("size_in_gb", Float, nullable=False),
    Column("parent_directory_id", Integer, ForeignKey("folders.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("file_state_id", Integer, ForeignKey("object_states.id", ondelete='SET NULL'),
           index=True, nullable=True),
    Column("file_scope_id", Integer, ForeignKey("object_scopes.id", ondelete='SET NULL'),
           index=True, nullable=True),
    Index('files_uix_1', 'name',
          'uploaded_by', 'parent_directory_id', unique=True))
...
files_starred = Table(
    ...
    Column("file_id", Integer, ForeignKey("files.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("starred_by",  UUID(as_uuid=True), ForeignKey("user.id", ondelete='CASCADE'),
           index=True, nullable=False),
    Column("is_starred", Boolean, default=True, nullable=False),
    Index('fs_uix_1', 'file_id',
          'starred_by', unique=True)
...
# Column("is_force_deleted", Boolean, default=False, nullable=False)
